<template>
    <div>
        <div class="form-group" :class="{ 'has-error': errors.name }">
            ...
            <input type="text" class="form-control" v-model="courier.name" :readonly="$route.name == 'outlets.edit'" placeholder="your name">
            <p class="danger" v-if="errors.name"> {{ errors.name[0] }}</p>
        </div>
        <div class="form-group" :class="{ 'has-error': errors.email }">
            <label for="">Email</label>
            <input type="text" class="form-control" v-model="courier.email" placeholder="your email">
            <p class="danger" v-if="errors.email"> {{ errors.email[0] }}</p>
        </div>
        <div class="form-group" :class="{ 'has-error': errors.password }">
            <label for=""> Password</label>
            <input type="password" class="form-control" v-model="courier.password" placeholder="your password">
            <p class="text-warning">Leave blank if you don't want to change password</p>
            <p class="danger" v-if="errors.password"> {{ errors.password[0] }}</p>
        </div>
        <div class="form-group" :class="{ 'has-error': errors.outlet_id }">
            ...
            <p class="danger" v-if="errors.outlet_id"> {{ errors.outlet_id[0] }}</p>
        </div>
        <div class="form-group" :class="{ 'has-error': errors.photo }">
            ...
            <p class="danger" v-if="errors.photo"> {{ errors.photo[0] }}</p>
        </div>
