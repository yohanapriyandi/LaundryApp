            ...
            <div class="panel-body">
                <b-table striped hover bordered :items="outlets.data" :fields="fields" show-empty="">
                    <template slot="status" slot-scope="row">
                        <span class="labe label-success" v-if="row.item.status == 1"> Active</span>
                        <span class="label label-default" v-else> Inactive</span>
                    </template>
                    <template slot="acions" slot-scope="row">
                        <router-link :to="{ name: 'outlets.edit', params: { id: row.item.code } }" class="btn-warning btn-sm"> <i class="fa fa-pencil"></i> </router-link>
